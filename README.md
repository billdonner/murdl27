...
- Daily: `DailyPuzzle` (MurdlCore) derives the day's answers per board count from the day number and a SplitMix64 seed, so every player gets the same boards; Daily #1 is 2026-09-18 local. `GameRecord.daily` marks Daily results; `ScoreSummary` adds daily played/won/streak (first result per day counts, replays never do). Share text says "MURDL Daily #N". Command-D on the Mac, calendar button or More menu on iOS.
- High contrast: `MurdlPalette.highContrast` swaps green/orange for orange/blue and tiles gain a check or arrow glyph; persisted in `MurdlHighContrast`. VoiceOver: boards are containers, tiles read row, letter, and state; keys read their letter state.
- Variants (`GameVariant`): Sequence reveals one board at a time (guesses = boards + 5 + one per four boards, `isBoardVisible`), Rescue pre-plays the Help starter openers before the clock starts. Records carry the variant. File > Variant on the Mac, More menu on iOS.
- Scores sync through iCloud key-value storage (`ScoreSync`, entitlement `com.apple.developer.ubiquity-kvstore-identifier`): union by record id, newest first, capped at 1,500 records; Clear stores a timestamp so cleared games stay cleared on every device. Scores also shows a guess distribution per board count.
...
- Universal (`TARGETED_DEVICE_FAMILY` 1,2): iPad all orientations, iPhone portrait only. The grid picks the column count with the biggest tiles among those that fill every row, so eight boards wrap into two rows of four in portrait. A hardware keyboard types through `.onKeyPress`; arrows move the board highlight.
...
- iPhone (`PagedBoardsView`): one board per page in a paged `TabView`, a numbered switcher strip under it (green solved, gray lost, ringed current), board count and mode in the More menu. The page is the game's focused board, so hardware arrows page too; after a guess finishes the board on screen the view moves to the next open one. Tall boards scroll inside the page and keep the typing row in view.
