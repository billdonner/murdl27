...
- Orange means the letter is in that board's answer, but in another position.
- Gray means the letter is not in that board's answer.

Solved boards stop changing. Keep using later guesses to finish the remaining boards.
...
Boards can be shown as a grid of up to eight per row, or as a single horizontal strip. Switch with the layout button or Command-L. When the boards do not fit, scroll with a two-finger swipe on the trackpad, a scroll wheel, or the arrow keys, which step the highlight from board to board and keep it in view. Click a board to highlight it.
